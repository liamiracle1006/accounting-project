...
class ReportService:
    ...
    def get_balance_sheet(self, as_of: date) -> BalanceSheet:
        """
        官方格式资产负债表（会企01表）。
        期初 = 当年 1 月 1 日之前的累计余额。
        """
        beg = date(as_of.year, 1, 1)
        # 期初余额取 1月1日 之前（即上年末）
    ...
    def get_income_statement(self, date_from: date, date_to: date) -> IncomeStatement:
        ...
        # 一、营业收入
        rev_c     = S("6001","CREDIT", date_from, date_to)
        rev_p     = S("6001","CREDIT", prev_from, prev_to)
        orev_c    = S("6051","CREDIT", date_from, date_to)
        orev_p    = S("6051","CREDIT", prev_from, prev_to)
        trev_c    = rev_c + orev_c
        trev_p    = rev_p + orev_p
        row("6001", "一、营业收入",                    trev_c, trev_p, True)

        # 减：营业成本
        cogs_c    = S("6401","DEBIT",  date_from, date_to)
        cogs_p    = S("6401","DEBIT",  prev_from, prev_to)
        row("6401", "减：营业成本",                    cogs_c, cogs_p)

        # 减：税金及附加
        tax_sur_c = S("6403","DEBIT",  date_from, date_to)
        tax_sur_p = S("6403","DEBIT",  prev_from, prev_to)
        row("6403", "减：税金及附加",                  tax_sur_c, tax_sur_p)

        # 减：销售费用
        sell_c    = S("6601","DEBIT",  date_from, date_to)
        sell_p    = S("6601","DEBIT",  prev_from, prev_to)
        row("6601", "减：销售费用",                    sell_c, sell_p)

        # 减：管理费用
        admin_c   = S("6602","DEBIT",  date_from, date_to)
        admin_p   = S("6602","DEBIT",  prev_from, prev_to)
        row("6602", "减：管理费用",                    admin_c, admin_p)

        # 减：财务费用
        fin_c     = S("6603","DEBIT",  date_from, date_to)
        fin_p     = S("6603","DEBIT",  prev_from, prev_to)
        row("6603", "减：财务费用",                    fin_c, fin_p)

        # 减：研发费用
        rd_c      = S("6604","DEBIT",  date_from, date_to)
        rd_p      = S("6604","DEBIT",  prev_from, prev_to)
        row("6604", "减：研发费用",                    rd_c, rd_p)

        # 加：公允价值变动收益
        fv_c      = S("6101","CREDIT", date_from, date_to)
        fv_p      = S("6101","CREDIT", prev_from, prev_to)
        row("6101", "加：公允价值变动收益",            fv_c, fv_p)

        # 加：投资收益
        inv_c     = S("6111","CREDIT", date_from, date_to)
        inv_p     = S("6111","CREDIT", prev_from, prev_to)
        row("6111", "加：投资收益",                    inv_c, inv_p)

        # 加：其他收益
        oth_c     = S("6117","CREDIT", date_from, date_to)
        ...
        # 加：营业外收入
        nonop_in_c = S("6301","CREDIT", date_from, date_to)
        nonop_in_p = S("6301","CREDIT", prev_from, prev_to)
        row("6301", "加：营业外收入",                  nonop_in_c, nonop_in_p)

        # 减：营业外支出
        nonop_ex_c = S("6711","DEBIT",  date_from, date_to)
        nonop_ex_p = S("6711","DEBIT",  prev_from, prev_to)
        ...
        # 减：所得税费用
        inc_tax_c = S("6801","DEBIT",  date_from, date_to)
        inc_tax_p = S("6801","DEBIT",  prev_from, prev_to)
