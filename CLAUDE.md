...
- **租户上下文**：**不要用 ContextVar / middleware 设 TenantContext**——会触发 `database/connection.py` 的 SQLAlchemy interceptor 给所有 ORM SELECT 注入 with_loader_criteria 过滤，跟历史 SQL 不兼容导致 500。每个需要 ctx 的路由用 `services/tenant_resolver.py:resolve_tenant_ctx(db, user)` 从 `current_user.tenant_id` 直接查（用原生 SQL，绕开 interceptor）
